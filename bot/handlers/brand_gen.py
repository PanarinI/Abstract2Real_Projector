...
async def stage1_problem(query: types.CallbackQuery, state: FSMContext):
    ...
    Комментарий: [краткий комментарий к выбору {username} и подводящий вопрос. 1-2 предложения. Обоснуй возможное проблемное поле и для ключевого слова приведи ссылку на релевантную и рабочую статью из Википедии (формат: <a href="URL">текст ссылки</a>]
    
    1. **[эмодзи]** [Проблема/Потребность 1 - 1-4 слова]: [Описание того, как данное решение отвечает на проблему (1-2 предложения)]
...
async def send_project_profile(query: types.CallbackQuery, state: FSMContext):
    ...
    Тэглайн: [одно предложение суть проекта]
...
async def receive_rating(query: types.CallbackQuery, state: FSMContext):
    ...
    await query.message.answer(
        ...
        "Теперь напишите короткий отзыв о проекте ⌨️"
    )
...
async def forward_feedback(message: types.Message, state: FSMContext):
    """
    Шаг 3. Получает текст отзыва и отправляет его в закрытую группу.
    """
